...
const Charts = ({ inventoryData, isDarkMode }) => {
  ...
  inventoryData.forEach((item) => {
    catMap[item.category] = (catMap[item.category] || 0) + (item.available ?? item.quantity);
  });
  ...
  const sorted = [...inventoryData].sort((a, b) => (b.available ?? b.quantity) - (a.available ?? a.quantity)).slice(0, 5);
